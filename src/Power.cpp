...
void Power_Init(void) {
	...
	#endif
	#if (POWER2 >= 0 && POWER2 <= MAX_GPIO)
		pinMode(POWER2, OUTPUT);     // Only necessary for GPIO. For port-expander it's done (previously) via Port_init()
	#endif
}
...
void Power_PeripheralOn(void) {
	...
	Port_Write(POWER2, POWER2_ON, false);
	#ifdef BUTTONS_LED
...
void Power_PeripheralOff(void) {
	...
	Port_Write(POWER2, POWER2_OFF, false);
	#ifdef BUTTONS_LED
