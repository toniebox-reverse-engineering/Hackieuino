    ...
    */
        ...
        #define HAL 0       // HAL 1 = LoLin32, 2 = ESP32-A1S-AudioKit, 3 = Lolin D32, 4 = Lolin D32 pro; ... 99 = custom
    #endif
    ...
    #ifdef I2C_2_ENABLE
        #define ext_IIC_CLK                 6           // i2c-SCL (clock)
        #define ext_IIC_DATA                5          // i2c-SDA (data)
    #endif

    // Power-control
    #define POWER                           45         // GPIO used to drive transistor-circuit, that switches off peripheral devices while ESP32-deepsleep
    #ifdef POWER
    ...
    #endif
    // Power-control 2 TODO
    #define POWER2                          47         // GPIO used to drive transistor-circuit, that switches off peripheral devices while ESP32-deepsleep
    #ifdef POWER2
        #define INVERT_POWER2                            // If enabled, use inverted logic for POWER circuit, that means peripherals are turned off by writing HIGH
    #endif
    //Board cfg END
    ...
    #define RFID_READER_TYPE_TRF7962A //TODO
    ...
    */
    ...
    // *****BUTTON*****        *****ACTION*****
    #define BUTTON_0_SHORT    CMD_NOTHING
    #define BUTTON_1_SHORT    CMD_NOTHING
    ...
    // (optinal) Neopixel
        ...
        constexpr float s_warningLowVoltage = 3.6;                      // If battery-voltage is <= this value, a cyclic warning will be indicated by Neopixel (can be changed via GUI!)
        constexpr float s_warningCriticalVoltage = 3.3;                 // If battery-voltage is <= this value, assume battery near-empty. Set to 0V to disable.
        constexpr float s_voltageIndicatorLow = 3.0;                    // Lower range for Neopixel-voltage-indication (0 leds) (can be changed via GUI!)
        constexpr float s_voltageIndicatorHigh = 4.5;                   // Upper range for Neopixel-voltage-indication (all leds) (can be changed via GUI!)
    #endif
    // (optional) Monitoring of battery-voltage via ADC
    #ifdef MEASURE_BATTERY_VOLTAGE
        #define VOLTAGE_READ_PIN            9          // GPIO used to monitor battery-voltage.
    ...
    #elif (HAL == 11)
        #include "settings-esp32-s3-toniebox.h"                     // Pre-configured settings for Raspiaudio ESPMuse Proto Board with I2C RFID Reader (https://raspiaudio.com/produit/muse-proto)
    #elif (HAL == 99)
